...
int** matrixMultiplication(int** lhs, int** rhs, int size)
{
	int **multiplication = createMatrix(size, size);
	int i, j, k;
	
	#pragma omp parallel for schedule(static) collapse(2) private(i, j, k) shared(lhs, rhs)
	for (i = 0; i < size; i++)
	{
		for (j = 0; j < size; j++)
		{
